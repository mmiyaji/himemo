...
class SettingsScreen extends ConsumerWidget {
  ...
  Future<void> _enterAdminMode(BuildContext context, WidgetRef ref) async {
    ...
    final controller = ref.read(adminModeSessionControllerProvider.notifier);
    List<String> missing;
    try {
      missing = await controller.unlock();
    } catch (_) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            strings.localized(
              en: 'Could not open admin mode. Please try again.',
              ja: '管理者モードを開けませんでした。もう一度お試しください。',
            ),
          ),
        ),
      );
        ...
        content: Text(
          needsPassword
              ...
              : strings.localized(
                  ...
                  ja: '管理者モードに入りました。全プロファイルのメモと添付を開けます。',
                ),
        ),
      ),
    );
  ...
  Future<bool> _prepareProfileForAdmin(
    BuildContext context,
    WidgetRef ref,
    String vaultId,
    String name,
  ...
  Future<void> _enterPrivateProfileFromAdmin(
    BuildContext context,
    WidgetRef ref,
    String vaultId,
    String name,
    ...
    final strings = context.strings;
    if (!await _prepareProfileForAdmin(context, ref, vaultId, name) ||
        !context.mounted ||
        !ref.read(adminModeSessionControllerProvider)) {
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        content: Text(
          strings.localized(
            en: 'Showing $name. Admin mode remains active.',
            ja: '$name を表示しています。管理者モードは継続中です。',
          ),
        ),
      ),
    );
                ...
                ListTile(
                  ...
                  subtitle: Text(
                    ref
                            ...
                            .isProfileUnlocked(legacyPrivateVaultId)
                        ? strings.localized(
                                      ...
                                      try {
                                        ...
                                        messenger.showSnackBar(
                                          SnackBar(
                                            ...
                                            content: Text(
                                              strings.localized(
                                                en: 'Could not download the attachments. Check your connection and cloud sign-in, then try again.',
                                                ja: '添付をダウンロードできませんでした。通信状態と同期先への接続を確認し、もう一度お試しください。',
                                              ),
                                            ),
                                          ),
                                        );
              ...
              ListTile(
                ...
                subtitle: Text(
                  storageDiagnosticsSummary.when(
                    data: (summary) => strings.localized(
                      en: 'Measured ${strings.byteCount(summary.measuredBytes)} / app container ${strings.byteCount(summary.appContainerBytes)}: Documents ${strings.byteCount(summary.documentsBytes)}, Library ${strings.byteCount(summary.libraryBytes)}, tmp ${strings.byteCount(summary.temporaryDirectoryBytes)}, other ${strings.byteCount(summary.appContainerOtherBytes)} / Library: App Support ${strings.byteCount(summary.appSupportBytes)}, Caches ${strings.byteCount(summary.cacheDirectoryBytes)}, other ${strings.byteCount(summary.libraryOtherBytes)} / SQLite ${strings.byteCount(summary.sqliteBytes)} / sync exports ${strings.byteCount(summary.syncExportBytes)} / App Group ${strings.byteCount(summary.appGroupBytes)}: shared imports ${strings.byteCount(summary.appGroupSharedImportsBytes)}, pending quick capture ${strings.byteCount(summary.appGroupPendingQuickCaptureBytes)} / preferences about ${strings.byteCount(summary.sharedPreferencesApproxBytes)}',
                      ja: '計測 ${strings.byteCount(summary.measuredBytes)} / app container ${strings.byteCount(summary.appContainerBytes)}: Documents ${strings.byteCount(summary.documentsBytes)}, Library ${strings.byteCount(summary.libraryBytes)}, tmp ${strings.byteCount(summary.temporaryDirectoryBytes)}, その他 ${strings.byteCount(summary.appContainerOtherBytes)} / Library: App Support ${strings.byteCount(summary.appSupportBytes)}, Caches ${strings.byteCount(summary.cacheDirectoryBytes)}, その他 ${strings.byteCount(summary.libraryOtherBytes)} / SQLite ${strings.byteCount(summary.sqliteBytes)} / 同期書き出し ${strings.byteCount(summary.syncExportBytes)} / App Group ${strings.byteCount(summary.appGroupBytes)}: shared imports ${strings.byteCount(summary.appGroupSharedImportsBytes)}, pending quick capture ${strings.byteCount(summary.appGroupPendingQuickCaptureBytes)} / 設定 約 ${strings.byteCount(summary.sharedPreferencesApproxBytes)}',
                      zh: '已测量 ${strings.byteCount(summary.measuredBytes)} / app container ${strings.byteCount(summary.appContainerBytes)}: Documents ${strings.byteCount(summary.documentsBytes)}, Library ${strings.byteCount(summary.libraryBytes)}, tmp ${strings.byteCount(summary.temporaryDirectoryBytes)}, 其他 ${strings.byteCount(summary.appContainerOtherBytes)} / Library: App Support ${strings.byteCount(summary.appSupportBytes)}, Caches ${strings.byteCount(summary.cacheDirectoryBytes)}, 其他 ${strings.byteCount(summary.libraryOtherBytes)} / SQLite ${strings.byteCount(summary.sqliteBytes)} / 同步导出 ${strings.byteCount(summary.syncExportBytes)} / App Group ${strings.byteCount(summary.appGroupBytes)}: shared imports ${strings.byteCount(summary.appGroupSharedImportsBytes)}, pending quick capture ${strings.byteCount(summary.appGroupPendingQuickCaptureBytes)} / 设置约 ${strings.byteCount(summary.sharedPreferencesApproxBytes)}',
                      ko: '측정 ${strings.byteCount(summary.measuredBytes)} / app container ${strings.byteCount(summary.appContainerBytes)}: Documents ${strings.byteCount(summary.documentsBytes)}, Library ${strings.byteCount(summary.libraryBytes)}, tmp ${strings.byteCount(summary.temporaryDirectoryBytes)}, 기타 ${strings.byteCount(summary.appContainerOtherBytes)} / Library: App Support ${strings.byteCount(summary.appSupportBytes)}, Caches ${strings.byteCount(summary.cacheDirectoryBytes)}, 기타 ${strings.byteCount(summary.libraryOtherBytes)} / SQLite ${strings.byteCount(summary.sqliteBytes)} / 동기화 내보내기 ${strings.byteCount(summary.syncExportBytes)} / App Group ${strings.byteCount(summary.appGroupBytes)}: shared imports ${strings.byteCount(summary.appGroupSharedImportsBytes)}, pending quick capture ${strings.byteCount(summary.appGroupPendingQuickCaptureBytes)} / 설정 약 ${strings.byteCount(summary.sharedPreferencesApproxBytes)}',
                      es: 'Medido ${strings.byteCount(summary.measuredBytes)} / app container ${strings.byteCount(summary.appContainerBytes)}: Documents ${strings.byteCount(summary.documentsBytes)}, Library ${strings.byteCount(summary.libraryBytes)}, tmp ${strings.byteCount(summary.temporaryDirectoryBytes)}, otros ${strings.byteCount(summary.appContainerOtherBytes)} / Library: App Support ${strings.byteCount(summary.appSupportBytes)}, Caches ${strings.byteCount(summary.cacheDirectoryBytes)}, otros ${strings.byteCount(summary.libraryOtherBytes)} / SQLite ${strings.byteCount(summary.sqliteBytes)} / exportaciones de sincronización ${strings.byteCount(summary.syncExportBytes)} / App Group ${strings.byteCount(summary.appGroupBytes)}: shared imports ${strings.byteCount(summary.appGroupSharedImportsBytes)}, pending quick capture ${strings.byteCount(summary.appGroupPendingQuickCaptureBytes)} / preferencias aprox. ${strings.byteCount(summary.sharedPreferencesApproxBytes)}',
                      de: 'Gemessen ${strings.byteCount(summary.measuredBytes)} / app container ${strings.byteCount(summary.appContainerBytes)}: Documents ${strings.byteCount(summary.documentsBytes)}, Library ${strings.byteCount(summary.libraryBytes)}, tmp ${strings.byteCount(summary.temporaryDirectoryBytes)}, Sonstiges ${strings.byteCount(summary.appContainerOtherBytes)} / Library: App Support ${strings.byteCount(summary.appSupportBytes)}, Caches ${strings.byteCount(summary.cacheDirectoryBytes)}, Sonstiges ${strings.byteCount(summary.libraryOtherBytes)} / SQLite ${strings.byteCount(summary.sqliteBytes)} / Sync-Exporte ${strings.byteCount(summary.syncExportBytes)} / App Group ${strings.byteCount(summary.appGroupBytes)}: shared imports ${strings.byteCount(summary.appGroupSharedImportsBytes)}, pending quick capture ${strings.byteCount(summary.appGroupPendingQuickCaptureBytes)} / Einstellungen ca. ${strings.byteCount(summary.sharedPreferencesApproxBytes)}',
                    ),
                    loading: () => strings.localized(
